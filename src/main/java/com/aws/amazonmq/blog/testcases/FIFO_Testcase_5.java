...
/**
 * This class demonstrates FIFO Test case # 5 where:
 *  - a queue has four producers 
 *  - a queue has three consumers
 *  - a queue has four message groups
...
public class FIFO_Testcase_5 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Logger logger = LoggerFactory.getLogger(FIFO_Testcase_5.class);
		...
		try {
			...
			JobDetail job4 = newJob(MsgProducer_FIFO.class)
					.withIdentity(useCaseId.concat("-").concat("producer-job4"), "group1")
					.usingJobData("producerName", "producer_4").usingJobData("queueName", queueName)
					.usingJobData("amazonMQSSLEndPoint", amazonMQSSLEndPoint).usingJobData("username", username)
					.usingJobData("password", password).usingJobData("useCaseId", useCaseId)
					.usingJobData("msgGroup", "group-4").usingJobData("numMsgs", numMsgs)
					.usingJobData("msgIdSequence", 400000).usingJobData("msgPrefix", "D-").build();
			...
			JobDetail job5 = newJob(MsgConsumer_CustomPrefetch.class)
					...
					.usingJobData("amazonMQSSLEndPoint", amazonMQSSLEndPoint).usingJobData("username", username)
					.usingJobData("password", password).usingJobData("useCaseId", useCaseId)
					.usingJobData("numMsgs", numMsgs * 4).usingJobData("prefetchSize", prefetchSize).build();
